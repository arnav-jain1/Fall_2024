...
int main (int argc, char *argv[])
{
   ...
   */
    ...
    int err = fstat(fdin, &statbuf);
    if (err < 0) {
        err_sys("Failed fstat\n");
    }

  /* 
   * 2. go to the location corresponding to the last byte 
   */
    err = lseek(fdout, statbuf.st_size, SEEK_SET);

    if (err < 0) {
        err_sys("Failed lseek\n");
    }
   ...
   */
    ...
    if (dst < 0) {
        err_sys("Failed mmap for output\n");
    }
